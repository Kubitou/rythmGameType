...
const judge = new Judge(noteManager, PERFECT_WINDOW, GOOD_WINDOW, BAD_WINDOW);
const tests = [
...
for (const test of tests) {
    const note = new TapNote(10, 1, "DON", "small");
    noteManager.load([note]);
    noteManager.update(10);
    const result = judge.tryHit(10 + test.offset, "DON");
    const success = result === test.expected;
    if (success) {
        console.log(`PASS  +${test.offset.toFixed(2)} → ${result}`);
        passed++;
    }
    else {
        console.error(`FAIL  +${test.offset.toFixed(2)} → esperado: ${test.expected}, recebido: ${result}`);
    }
...
//# sourceMappingURL=testJudge.js.map